...
- scripts section: `data/maps/<MapName>/scripts.inc` (always include this script bank entry point)
- text section: `data/maps/<MapName>/text.inc` only when the map still owns separate text labels in `text.inc`

If a map uses `scripts.pory`, keep including `scripts.inc` here; it is generated by the Make rule.
`mapjson` does not auto-register script/text include lines in this file.
